...
class InsightContextService:
    ...
    def _sales_insight_data(self, period_days: int) -> str:
        orders = execute_read_query(
            ...
            "FROM orders "
            f"WHERE created_at >= CURRENT_DATE - INTERVAL '{period_days} days'"
        )
        ...
        top_products = execute_read_query(
            ...
            "FROM order_items oi "
            "JOIN chemicals ch ON oi.chemical_id = ch.id "
            "JOIN orders o ON oi.order_id = o.id "
            f"WHERE o.created_at >= CURRENT_DATE - INTERVAL '{period_days} days' "
            "GROUP BY ch.id, ch.chemical_name ORDER BY revenue DESC LIMIT 10"
        )
    ...
    def _inventory_insight_data(self, period_days: int) -> str:
        ...
        low_stock = execute_read_query(
            ...
            "FROM chemicals WHERE quantity <= min_quantity ORDER BY quantity ASC"
        )
    ...
    def get_quick_insights_data(self) -> str:
        ...
        stats = execute_read_query(
            ...
            "(SELECT COUNT(*) FROM chemicals WHERE quantity <= min_quantity) as low_stock, "
            ...
            "(SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE payment_status = 'unpaid') as unpaid_amount"
        )
